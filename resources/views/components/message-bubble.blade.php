<div
    ...
    <div
        ...
        <div class="flex h-full gap-5">
                            ...
                            <div class="flex flex-col items-end ">
                                <div class="flex items-center gap-2 text-xs text-gray-500">
                                    <a href="{{ asset('storage/uploads/' . $message->body) }}" target="_blank"
                                        ...
                                        <span class="ml-1 dark:text-white">{{ pathinfo($message->body, PATHINFO_BASENAME) }}</span>
                                    </a>
                                </div>
                            </div>
                                ...
                                <div class="flex items-center gap-2 text-xs text-gray-500">
                                    <a href="{{ asset('storage/uploads/' . $message->body) }}" target="_blank"
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke-width="1" stroke="currentColor" class="size-5 dark:text-white">
            ...
            @if (Str::endsWith($message->body, ['jpg', 'jpeg', 'png', 'gif', 'webp']))
            @else
                <div class="flex items-center self-end gap-1 mt-1 text-xs text-gray-500 dark:text-gray-200">
                    <span>{{ $message->created_at->format('H:i') }}</span>
                    @if ($isCurrentUser)
                        <x-chat-check class="dark:text-gray-200" :message="$message" />
                    @endif
                </div>
            @endif
        </div>
